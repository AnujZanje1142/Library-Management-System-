...
class MainWin(Tk):
    def __init__(self):
        ...
        self.canvas.pack()
        self.photo = PhotoImage(file='pexels-игорь-альшин-2873277.png')
        self.canvas.create_image(-20, -20, image=self.photo, anchor=NW)
        self.maxsize(1320, 768)
        self.minsize(1320,768)
        self.state('zoomed')
        self.title('SILVER OAK LIBRARY MANAGEMENT SYSTEMS(SOULMS)')
        self.a = StringVar()
        self.b = StringVar()
...
#creating table
        ...
        def ser():
            try:
                ...
                self.myCursor.execute("Select issue.BID,issue.SID,students.name,students.Fine,books.Book_name,issue.Issue_date,issue.Return_date from books,students,issue where issue.BID = issue.issue.book and SID = ?",[self.change])
        ...
        def ent():
            try:
                self.conn = sqlite3.connect('library_administration.db')
                self.myCursor = self.conn.cursor()
                self.myCursor.execute("Select issue.BID,issue.SID,students.name,students.Fine,books.Book_name,issue.Issue_date,issue.Return_date from books,students, issue where issue.BID = issue.Book and BID = ?",[self.b.get()])
                self.pc = self.myCursor.fetchall()
                if self.pc:
                    self.listTree.delete(*self.listTree.get_children())
                    for row in self.pc:
        ...
        def check():
            try:
                ...
                if not z:
                    ...
                    #label and input box
                    self.label3 = Label(self, text='SILVER OAK LIBRARY MANAGEMENT SYSTEM',fg='white',bg="black" ,font=('AlGERIAN', 30, 'bold'))
                    self.label3.place(x=147, y=22)
                    self.label4 = Label(self, text="ENTER STUDENT",fg='white',bg="black",  font=('Arial', 18, 'bold'))
                    ...
                    self.label5 = Label(self, text="ENTER THE BOOK ID",fg='white',bg="black" , font=('Arial', 18, 'bold'))
                    ...
                    self.label6 = Label(self, text="INFORMATION DETAILS",fg='white',bg="black" ,  font=('Arial', 15, 'underline', 'bold'))
